...
import { ConnectFourSFactory, ConnectFourS, ConnectFour__factory, ConnectFourSFactory__factory } from "../typechain"
describe("ConnectFourS Seasons", () => {
  let ConnectFourSFactory: ConnectFourSFactory
  let [account1, account2, account3]: SignerWithAddress[] = []

  let [gameOneContractSignerOne, gameOneContractSignerTwo, gameOneContractSignerThree]: ConnectFourS[] = []
  ...
  beforeEach(async () => {
    ...
    ConnectFourSFactory = await new ConnectFourSFactory__factory(account1).deploy(
      connectFourContractImpl.address
    )

    const response = await (await ConnectFourSFactory.deployNewSeason()).wait()
